const ContactDetailsCard = ({ color, icon, title, desc }) => {
  return (
    ...
    >
      ...
      <h3 className="py-2 font-gluten text-2xl font-bold">{title}</h3>
      <p className="w-full break-words text-center font-lexend text-sm font-semibold leading-4">{desc}</p>
    </div>
  )
}
