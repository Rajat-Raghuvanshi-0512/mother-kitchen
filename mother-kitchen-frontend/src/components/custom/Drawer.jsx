const Drawer = ({ isOpen, onClose, children, left, title = 'Drawer Title' }) => {
  return (
    <div
      className={`${
        isOpen ? 'translate-x-0' : left ? '-translate-x-full' : 'translate-x-full'
      } w-screen h-screen fixed z-50 top-0 left-0 duration-500 bg-opacity-20 transition-transform`}
    >
      <div className={`${left ? 'left-0' : 'right-0'} top-0 fixed w-2/3 md:max-w-lg`}>
